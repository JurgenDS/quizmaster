...
const TableCells = ({ cells }: TableCellsProps) => (
    <tr>
        {cells.map(cell => <td>{cell.value}</td>)}
    </tr>
)
...
export const TableHead = ({ cells }: TableHeadProps) => (
    <thead>
        <tr>
            {cells.map(cell => <th>{cell.value}</th>)}
        </tr>
    </thead>
)
...
export const TableBody = ({ rows }: TableBodyProps) => (
    <tbody>
        {rows.map(row => <TableCells cells={row.cells} />)}
    </tbody>
)
...
export const DataTableDoc = ({ dataTable }: DataTableDocProps) => {
    ...
    const [header, rows] = hasHeader
        ? [dataTable.rows[0].cells, dataTable.rows.slice(1)]
        : [undefined, dataTable.rows]
